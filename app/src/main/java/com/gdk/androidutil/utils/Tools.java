...
public class Tools {
    ...
    /**
     * 18位或者15位身份证验证 18位的最后一位可以是字母x，身份证校验工具类
     *
     * @param text
     * @return
     */
...
//		return text.matches(regx) || text.matches(reg1) || text.matches(regex) || text.matches(regx1);
    ...
    public static boolean passWordIdentification(String passWord) {
        if (null == passWord) {
            return false;
        }
    ...
    public static boolean validateBankAcct(String acctNo) {
        if (null == acctNo) {
            return false;
        }
        if (acctNo.length() < 10) {
    ...
    public static boolean validateBankAcct_22(String acctNo) {
        if (null == acctNo) {
            return false;
        }
        if (acctNo.length() > 25) {
    ...
    public static String divide(String v, int scale) {
        ...
        BigDecimal divide = b.divide(one, scale, BigDecimal.ROUND_HALF_UP);
